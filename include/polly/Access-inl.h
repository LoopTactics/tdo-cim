...
static inline bool operator==(const isl::space &l, const isl::space &r) {
    ...
    for (size_t pos : group) {
      ...
      if (matchedSpace) { // A group has already matched a map.
        ...
        if (it != previouslyMatchedSpaces.end()) {
          // WARNING (set to false to make stencil example works)
          return false;
        }
        // LOG(INFO) << "matched space :" << matchedSpace.to_str() << "\n";
...
static bool areIdEqual(const isl::id &l, const isl::id &r) {
  return l.get() == r.get();
}
...
bool compareGroupsBelongToSameArray(
  ...
  return (areIdEqual(id1, id2)) ^ !equality;
}
...
Matches<typename PlaceholderCollectionTy::CandidateTy,
        typename PlaceholderCollectionTy::PatternTy>
match(std::vector<isl::map> accesses, PlaceholderCollectionTy ps) {
