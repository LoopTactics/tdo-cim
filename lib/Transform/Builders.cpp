...
/// Constructor. It allows to create a new band
/// node with the same properties of the original
/// (i.e., the captured one) but with different
/// schedule. It is usefull if for example you apply
/// tiling and you want to mantain the node properties
/// for the new nodes.
BandDescriptor::BandDescriptor(isl::multi_union_pw_aff mupa,
                               isl::schedule_node band) {
  partialSchedule = mupa;
  int n = partialSchedule.dim(isl::dim::set);
  for (int i = 0; i < n; i++) {
    coincident.push_back(band.band_member_get_coincident(i));
  }
  permutable = band.band_get_permutable();
}
...
ScheduleNodeBuilder subtreeBuilder(isl::schedule_node node) {
  ...
  if (type == isl_schedule_node_domain) {
    ...
    // consider also node attributes (if any) when re-building.
    builder = band([node]() {
      auto descr = BandDescriptor(node.band_get_partial_schedule());
      int dim = node.band_get_partial_schedule().dim(isl::dim::set);
      std::vector<bool> currentCoincident;
      for (int i = 0; i < dim; i++) {
        if (node.band_member_get_coincident(i))
          currentCoincident.push_back(true);
        else
          currentCoincident.push_back(false);
      }
      bool currentPermutable = false;
      if (node.band_get_permutable())
        currentPermutable = true;
      descr.permutable = currentPermutable;
      descr.coincident = currentCoincident;
      return descr;
    });
